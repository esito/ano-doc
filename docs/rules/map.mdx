...
- Run a `map` Work Task only **once** on a column of a data set
  - Running a `map` rule **twice** on the same column in the same data set may result in **data integrity problems**
    ...
    - After the second run, in the cases of `output` and `input-output`,
      the mapping file created/updated will no longer properly map real production values to anonymized values
...
You may encrypt the mapping file using the `encrypted` keyword, and provide your own encryption key when [configuring the application](../from-ano-to-execution/configure).
