...
The `mask` rule generates values for the column fields.

- In an `update`, it will replace existing column values
- In a `create`, it will populate new rows of values
