...
- Choose a `<distribution>` function that decides how parent table values will be distributed in the child column. E.g. that all parent values must appear at least once.
...
- a desired distribution of rows in a table, based upon the occurence of values in other tables
...
The number of rows will be at least the mulitplication value of the number of rows of each parent.
