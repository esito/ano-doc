...
For other formats see [formats](../format-output).

## `convert <conversion>` - Convertion functions for randomize
...
Replace `<conversion>` with correct conversion function, defined by the type:
...
`percentage-noise` allows addition of a randomized percentage of the original value. The given percentage of the original value is multiplied with a random number before addition.
