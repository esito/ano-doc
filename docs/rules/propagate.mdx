...
- there are a foreign key constraint(s) to the column you are anonymizing
...
Note that `Address` have a foreign key constraint to `Customer`, so every `Address.customer_customerNo` value must exist in `Customer.customerNo`
...
- When they exists, then update the values through `propagate`
