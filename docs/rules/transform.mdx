...
- **Custom Transformation:** If you use a custom transformation and not an built in one, then import it in the `.ano` file using the `transformation` keyword
...
The `Email` builtin transformation function does the following to the format string output:
...
The `CreditCard` builtin transformation function uses the Luhn algorithm for Credit Card encoding. It replaces the last digit for MOD10 checksum validation.
