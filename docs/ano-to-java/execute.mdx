...
Use the `task` command to list the available tasks. Use the `run` command to execute all the tasks. Use the `run <task-name>` to execute specific Task Groups or Work Tasks.
