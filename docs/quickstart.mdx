...
Use our [DBano Service](from-sql-to-ano/translate-to-ano) to translate your database definitions by providing it the `.sql` file. [Register and Login to gain access](services/access).
...
[Use our DBmasker Service](from-ano-to-execution/generate) to generate the runtime project by providing the `.ano` file. [Buy DBmasker and Login to gain access](services/access). The service will provide you an `.zip` file containing a Java 8, Maven 3.6 project.
