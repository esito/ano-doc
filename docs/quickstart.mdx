...
:::tip Goal
This Quickstart will explain the Anonymizer workflow:
...
This allows you to test your anonymization logic against real data each time you write and run new versions of the Tasks and Rules.
...
Use our [DBano Service](from-sql-to-ano/translate-to-ano) to translate your database definitions by providing it the `.sql` file. [Register and log in to gain access](services/access).l
...
In the `.ano` file we now write the logic for our desired anonymizations. Consult the [`Writing Tasks`](tasks/overview-tasks) and [`Writing Rules`](rules/overview-rules) sections for details on how to do this.
...
[Use our DBmasker Service](from-ano-to-execution/generate) to generate the runtime project by providing the `.ano` file. [Buy DBmasker and log in to gain access](services/access). The service will provide you an `.zip` file containing a Java 8, Maven 3.6 project.
...
Configure the Java 8 application by [using this guide](from-ano-to-execution/configure). There are two required steps. First, you must add the Maven Dependency that properly connects the Java application to your database. Second, you must provide the Java application the database URL and login credentials.
