...
Therefore, to start writitng with ANO Tasks and Rules, it is a necessary step to desrcibe the database in the ANO Language.

:::

## The ANO Language

[The ANO Language](the-ano-language) is a Domain Specific Language created for the purpose of anonymization. It uses [its own ANO types](ano-types) that are database agnostic.

## Translating from SQL to ANO

- [Use DBano to translate DDL statements in SQL to ANO](translate-to-ano)

When this is done, you may freely write [ANO Tasks](../tasks/overview-tasks) and [ANO Rules](../rules/overview-rules) according to [the ANO Language](the-ano-language).
