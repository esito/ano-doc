...
Task Groups are collections of Work Tasks. Work Tasks are collections of [anonymization rules](../rules/overview-rules). A Work Task points to a specific table. A rule within points to a specifc column.
...
- The order of task run is the same as the order of the task in the ANO file
...
- Naming two Task Groups the same renders them indistinguishable when running

## Work Task Types

A **Work Task** is organized by database table. The Work Task type defines what kind of [`Rules`](../rules/overview-rules) can be applied.
...
- Unique keys using the [`unique`](../rules/overview-rules#unique) keyword
...
See [update](work-task-update) for more details.
...
See [create](work-task-create) for more details.
